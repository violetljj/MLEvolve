...
def main() -> None:
    ...
    if not 1 <= args.concurrency <= 32:
        raise SystemExit("concurrency must be within 1..32")
    ...
    ssh = ["ssh", "-p", str(args.port), "-o", "BatchMode=yes", args.host]
    ...
    with tempfile.TemporaryDirectory(prefix="remote-worker-") as temporary:
        ...
        try:
            ...
            remote_command = (
                ...
                f"--python '{args.remote_python}' --concurrency {args.concurrency} --timeout {args.timeout}"
            )
            ...
            if args.cleanup_remote:
                run(ssh + [f"test '{remote_run}' != / && rm -rf -- '{remote_run}'"])
